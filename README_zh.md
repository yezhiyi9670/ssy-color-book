...
可以通过此页顶部链接访问**网页版色卡**或下载 **RGB 颜色值**。

- 在**网页版色卡**中，可以左键点击颜色查看详情，此时可以直接按 Ctrl+C 复制颜色编号；按右键也会打开详情，此时按 Ctrl+C 复制当前色域下的十六进制数值。
- **RGB 颜色值**文件可用于某些设计软件。例如，`scl` 文件可用于 Style3D 系列软件，而 `gpl` 文件可以用于许多绘图软件，包括 GIMP 和 Inkscape。
- 使用浏览器的查找功能可以在网页版色卡中根据颜色编号查找颜色。也可以输入十六进制数值反查颜色编号。
- 若要离线使用网页版色卡，可以直接在网页上按 Ctrl+S 下载网页。
